...
public class CustomerMasterDao {
	...
	public List<CustomerMaster> selectAll() throws SQLException {
		...
		String sql ="select customer_id ,customer_code, customer_name, ifnull(product_group,'')as product_group,  ifnull(SalesPerson,'') as SalesPerson, ifnull(instruction,'') as instruction, ifnull(sap,'')as sap from fms.customer_master";
	...
	public CustomerMaster getDataById(int id) {
		...
		try {
			String sql = "select customer_code, customer_name, ifnull(product_group,'') as product_group, ifnull(SalesPerson,'') as SalesPerson, ifnull(instruction,'') as instruction, ifnull(sap,'') as sap from fms.customer_master where customer_id=?";
			...
			while (rs.next()) {
				...
				cus.setCustomerName(rs.getString(2));
				cus.setProductGroup(rs.getString(3));
				cus.setSalesRep(rs.getString(4));
				cus.setRemarks(rs.getString(5));
				cus.setSap(rs.getString(6));
				cus.setId(id);
